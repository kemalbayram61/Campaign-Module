...
class Campaign:
    ...
    def dict_to_campaign(dict_data: dict) -> object:
        response = Campaign(id=str(dict_data["id"]),
                            ...
                            level=int(dict_data["level"]) if dict_data["level"] != None else None,
                            min_qty=int(dict_data["min_qty"]) if dict_data["min_qty"] != None else None,
                            min_amount=float(dict_data["min_amount"]) if dict_data["min_amount"] != None else None,
                            max_discount=float(dict_data["max_discount"]) if dict_data["max_discount"] != None else None,
                            max_occurrence=int(dict_data["max_occurrence"]) if dict_data["max_occurrence"] != None else None,
                            action_type=ActionType.AMOUNT if int(dict_data["action_type"])==0 else ActionType.PERCENT,
                            action_amount=float(dict_data["action_amount"]) if dict_data["action_amount"] != None else None,
