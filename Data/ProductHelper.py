...
class ProductHelper(DBObject):
    ...
    def __fetch_on_db(self) -> None:
        ...
        if self.id is not None:
            ...
            if self.org_id is None:
                ...
                db_object = db_helper.find_product_by_barcode(self.barcode, self.org_id)
    ...
    def __fetch_on_redis(self) -> None:
        product_list: list[Product] = self.get_all(self.org_id)
    ...
    def load_data(self) -> None:
        ...
        redis_helper.set("product_list_" + self.org_id, product_list_str)
        ApplicationCacheHelper.get_instance().store_data("product_list_" + self.org_id, product_list)

    def get_all(self, org_id: str) -> list[Product]:
        response: list[Product] = []
        if self.role == DBObjectRole.DATABASE:
            db_helper: DBHelper = DBHelper()
            db_object_list = db_helper.select_all("product", org_id)
            ...
            redis_helper: RedisHelper = RedisHelper()
            product_list_str: str = str(redis_helper.get("product_list_" + org_id))
            ...
            response = ApplicationCacheHelper.get_instance().get_data("product_list_" + org_id)
        return response